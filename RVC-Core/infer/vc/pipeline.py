...
from rvc_core.device import require_mps_tensor, synchronize
from infer.vc.singing import (
    SINGING_MODE,
    SINGING_NOISE_SCALE,
    normalize_inference_mode,
    process_singing_f0,
    smooth_retrieval_features,
)
...
class Pipeline(object):
    ...
    def get_f0(
        ...
        if f0_method == "pm":
            ...
            if mode == SINGING_MODE:
                f0, f0_confidence = self.model_rmvpe.infer_from_audio(
                    x, thred=0.03, return_confidence=True
                )
            else:
        ...
        if mode == SINGING_MODE:
            f0 = process_singing_f0(f0, f0_confidence)
    ...
    def vc(
        ...
        require_mps_tensor(feats, "HuBERT feature extraction")
        ...
        if (
            ...
            if mode == SINGING_MODE and pitchf is not None:
                singing_f0 = pitchf[0].detach().float().cpu().numpy()
                npy = smooth_retrieval_features(npy, singing_f0)
        ...
        with torch.no_grad():
            ...
            if hasp:
                if mode == SINGING_MODE:
                    synthesized = run_cuda_graph(
                        ...
                        lambda phone, lengths, coarse, continuous, speaker: net_g.infer(
                            ...
                            continuous,
                            speaker,
                            noise_scale=SINGING_NOISE_SCALE,
                        )[0],
                        feats,
                        p_len,
                        pitch,
                ...
                if mode == SINGING_MODE:
                    synthesized = run_cuda_graph(
                        net_g,
                        "rvc-synth-no-f0-singing",
                        lambda phone, lengths, speaker: net_g.infer(
                            phone, lengths, speaker, noise_scale=SINGING_NOISE_SCALE
                        )[0],
                        feats,
                        p_len,
                        sid,
            ...
            require_mps_tensor(synthesized, "RVC synthesis")
            synchronize(self.device)
