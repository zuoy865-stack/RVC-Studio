...
from rvc_core.device import require_mps_module, require_mps_tensor, synchronize
...
class RMVPE:
    def __init__(self, model_path, is_half, device=None):
        ...
        if "privateuseone" in str(device):
            ...
            if str(torch.device(device).type) == "mps":
                require_mps_module(self.model, "RMVPE model")
        ...
        with torch.no_grad():
            ...
            if "privateuseone" in str(self.device):
                ...
                if torch.device(self.device).type == "mps":
                    require_mps_tensor(hidden, "RMVPE pitch inference")
    ...
    def infer_from_audio(self, audio, thred=0.03, return_confidence=False):
        ...
        # 默认行为保持不变；唱歌模式可额外使用 RMVPE 每帧最大显著度作为置信度。
        confidence = None
        if return_confidence:
            confidence = np.max(hidden, axis=1).astype(np.float32)
