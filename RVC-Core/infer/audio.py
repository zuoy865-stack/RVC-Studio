...
# RVC-Core 根目录。当前文件位于 RVC-Core/infer/audio.py。
_CORE_ROOT = Path(__file__).resolve().parent.parent
_BUNDLED_FFMPEG = _CORE_ROOT / "runtime" / "ffmpeg" / "ffmpeg"
_BUNDLED_FFPROBE = _CORE_ROOT / "runtime" / "ffmpeg" / "ffprobe"


def _resolve_media_tool(tool_name):
    """优先解析 RVC-Core 内置的 FFmpeg/FFprobe，避免依赖 GUI 进程的 PATH。"""
    if tool_name == "ffmpeg":
        bundled = _BUNDLED_FFMPEG
        env_name = "RVC_FFMPEG"
    elif tool_name == "ffprobe":
        bundled = _BUNDLED_FFPROBE
        env_name = "RVC_FFPROBE"
    else:
        raise ValueError("Unsupported media tool: %s" % tool_name)

    # 发布版第一优先使用 RVC-Core/runtime/ffmpeg 中的内置工具。
    ...
    if env_path:
        env_tool = Path(env_path).expanduser()
        if env_tool.is_file() and os.access(env_tool, os.X_OK):
            return str(env_tool)

    # 最后仅作为开发环境兼容，尝试系统 PATH。
...
def _probe_audio(file):
    info = ffmpeg.probe(file, cmd=get_ffprobe_path())
    stream = next(
...
def _decode_audio_ffmpeg(file):
    ...
    out, _ = (
        ...
        .run(cmd=[get_ffmpeg_path(), "-nostdin"], capture_stdout=True, capture_stderr=True)
    )
